...
/// 这是轴基图数据的基类，其中包含一个 [FlGridData]，其中包含用于显示网格线的数据，我们还有
/// [minX]、[maxX]、[minY]、[maxY] 值，我们用它们来确定图表的比例是多少，并根据比例计算 x 和 y。
/// 每个子项都必须在他们的构造函数中设置它。
...
/// 它为您提供轴值，并基于该值获取 String 值。
typedef GetTitleWidgetFunction = Widget Function(double value, TitleMeta meta);

/// 默认的 [SideTitles.getTitlesWidget] 函数。
///
/// 使用 formatNumber 将轴编号格式化为较短的字符串。
...
class AxisTitles with EquatableMixin {
  /// you can provide [axisNameWidget] if you want to show a general
  /// label on this axis,
  ///
  /// [axisNameSize] determines the maximum size that [axisNameWidget] can use
  ///
  ...
  /// Determines the size of [axisNameWidget]
  ...
  /// If titles are showing on top of your tooltip, you can draw them below
  /// everything.
  ///
  /// In the future, we will convert tooltips to a widget, that would solve
  /// this problem.
...
class FlGridData with EquatableMixin {
  ...
  /// Representation of each line determines by [getDrawingHorizontalLine]
  /// callback, it gives you a double value (in the y axis), and you should
  /// return a [FlLine] that represents a horizontal line.
  /// You are allowed to show or hide any horizontal line, using
  /// [checkToShowHorizontalLine] callback, it gives you a double value
  /// (in the y axis), and you should return a boolean that determines
  /// showing or hiding specified line.
  /// or you can hide all horizontal lines by setting [drawHorizontalLine]
  /// false.
  ///
  ...
  /// Representation of each line determines by [getDrawingVerticalLine]
  /// callback, it gives you a double value (in the x axis), and you should
  /// return a [FlLine] that represents a horizontal line.
  /// You are allowed to show or hide any vertical line, using
  /// [checkToShowVerticalLine] callback, it gives you a double value
  /// (in the x axis), and you should return a boolean that determines
  /// showing or hiding specified line.
  ...
  /// Determines interval between horizontal lines, left it null to be auto
  /// calculated.
  ...
  /// Gives you a y value, and gets a boolean that determines showing or hiding
  /// specified line.
  ...
  /// Gives you a x value, and gets a boolean that determines showing or hiding
  /// specified line.
...
FlLine defaultGridLine(double value) {
  return FlLine(strokeWidth: 0.5, color: Colors.grey.withOpacity(0.5));
}
...
class FlLine with EquatableMixin {
  ...
  const FlLine({
    ...
    this.dashArray,
            ...
            color ?? ((color == null && gradient == null) ? Colors.grey : null);
...
class RangeAnnotations with EquatableMixin {
  ...
  /// using [horizontalRangeAnnotations], and [verticalRangeAnnotations]
  /// respectively.
...
class HorizontalRangeAnnotation with EquatableMixin {
  /// Annotates a horizontal region from most left to most right point of the
  /// chart, and from [y1] to [y2], and fills the area with [color] or
  /// [gradient].
  ...
  /// Lerp a [HorizontalRangeAnnotation] based on [t] value, check [Tween.lerp].
  ...
  /// Copies current [HorizontalRangeAnnotation] to a new
  /// [HorizontalRangeAnnotation], and replaces provided values.
...
class VerticalRangeAnnotation with EquatableMixin {
  /// Annotates a vertical region from most bottom to most top point of the
  /// chart, and from [x1] to [x2], and fills the area with [color] or
  /// [gradient].
  ...
  /// Copies current [VerticalRangeAnnotation] to a new
  /// [VerticalRangeAnnotation], and replaces provided values.
...
///
/// [LineChart] draws some [HorizontalLine] (set by
/// [LineChartData.extraLinesData]), in below or above of everything, it draws
/// from left to right side of the chart.
class HorizontalLine extends FlLine with EquatableMixin {
  ...
  ///
  /// You can have a dashed line by filling [dashArray] with dash size and
  /// space respectively.
  ///
  /// It draws an image in left side of the chart, use [sizedPicture] for
  /// vectors, or [image] for any kind of image.
  HorizontalLine({
    required this.y,
...
///
/// [LineChart] draws some [VerticalLine] (set by
/// [LineChartData.extraLinesData]), in below or above of everything, it draws
/// from bottom to top side of the chart.
class VerticalLine extends FlLine with EquatableMixin {
  ...
  /// space respectively.
  ///
  /// It draws an image in bottom side of the chart, use [sizedPicture] for
  /// vectors, or [image] for any kind of image.
  VerticalLine({
    required this.x,
...
class HorizontalLineLabel extends FlLineLabel with EquatableMixin {
  /// Draws a title on the [HorizontalLine], align it with [alignment] over the
  /// line, applies [padding] for spaces, and applies [style for changing color,
  /// size, ... of the text.
...
class VerticalLineLabel extends FlLineLabel with EquatableMixin {
  /// Draws a title on the [VerticalLine], align it with [alignment] over the
  /// line, applies [padding] for spaces, and applies [style for changing color,
  /// size, ... of the text.
  /// Drawing text will retrieve through [labelResolver],
  /// you can override it with your custom data.
...
class ExtraLinesData with EquatableMixin {
  ...
  ///
  /// If [extraLinesOnTop] sets true, it draws the line above the main bar
  /// lines, otherwise it draws them below the main bar lines.
...
class FlDotCrossPainter extends FlDotPainter {
  /// The [color] and [width] properties determines the color and thickness of
  /// the cross shape, [size] determines the width and height of the shape.
  FlDotCrossPainter({
    this.color = Colors.green,
    this.size = 8.0,
    this.width = 2.0,
