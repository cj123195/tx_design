...
abstract class RenderBaseChart<R extends BaseTouchResponse> extends RenderBox
  ...
  /// We use [FlTouchData] to retrieve [FlTouchData.touchCallback] and
  /// [FlTouchData.mouseCursorResolver] to invoke them when touch happens.
  ...
  /// Recognizes longPress gestures, such as onLongPressStart,
  ...
  /// Feeds our gesture recognizers for handling events, we also handle
  /// [PointerHoverEvent] here.
  ///
  /// Our gesture recognizers are responsible for notifying us about happened
  /// gestures (such as tap, panMove, ...)
  /// we need to give them [PointerDownEvent] then they will listen to the
  /// global [GestureBinding] for further events.
  ///
  /// We need to handle [PointerHoverEvent] because there is no gesture
  /// recognizer for mouse hover events (in fact they don't have any gestures,
  ...
  ///
  /// We get a [BaseTouchResponse] using [getResponseAtLocation] for events
  /// which contains a localPosition.
  ...
  /// [SystemMouseCursors.click] when mouse hovers a specific point of our
  /// chart.
  @override
  ...
  /// Charts need to implement this class to tell us what [BaseTouchResponse]
  /// is available at provided [localPosition]
  /// When touch/pointer event happens, we send it to the user alongside the
  /// [FlTouchEvent] using [_touchCallback]
