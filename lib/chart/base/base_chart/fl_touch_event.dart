...
abstract class FlTouchEvent {
  ...
  bool get isInterestedForInteractions {
    ...
    /// In desktop when mouse hovers into a chart element using
    /// [FlPointerHoverEvent], we show the interaction and when tap happens at
    /// the same position, interaction will be dismissed because of
    /// [FlTapUpEvent]. That's why we exclude it on desktop or web
...
/// When the pointer that previously triggered a [FlPanStartEvent] did not
/// complete.
...
/// When the pointer that previously triggered a [FlTapDownEvent] will not end
/// up causing a tap.
