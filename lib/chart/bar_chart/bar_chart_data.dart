...
import '../lerp.dart';
import '../utils.dart';
import 'bar_chart.dart' show BarChart;
...
class BarChartData extends AxisChartData with EquatableMixin {
  ...
  /// using axisTitleData, you can restrict the y axis using [minX], and
  /// [maxY] values.
  ///
  ...
  ///
  /// Horizontal lines are drawn with [extraLinesData]. Vertical lines will not
  /// be painted if received.
  /// Please see issue #1149 (https://github.com/imaNNeo/fl_chart/issues/1149)
  /// for vertical lines.
  BarChartData({
  ...
  /// [BarChart] draws [barGroups] that each of them contains a list of
  /// [BarChartRodData].
...
/// defines arrangement of [BarChartData.barGroups], check [MainAxisAlignment]
/// for more details.
...
class BarChartGroupData with EquatableMixin {
  /// [BarChart] renders groups, and arrange them using
  /// [BarChartData.alignment], [x] value defines the group's value in the x
  /// axis (set them incrementally).
  /// it renders a list of [BarChartRodData] that represents a rod (or a bar)
  /// in the bar chart, and applies [barsSpace] between them.
  ///
  ...
  /// [BarChart] renders [barRods] that represents a rod (or a bar) in the bar
  /// chart.
  final List<BarChartRodData> barRods;

  /// [BarChart] applies [barsSpace] between [barRods] if [groupVertically] is
  /// false.
...
class BarChartRodData with EquatableMixin {
  ...
  /// rounding corners and also [borderSide] for stroke border.
  /// Optionally you can use [borderDashArray] if you want your borders to have
  /// dashed lines.
  ///
  ...
  /// [backDrawRodData], it uses to have a bar with a passive color in rear of
  /// the rod, for example you can use it as the maximum value place holder.
  ///
  /// If you are a fan of stacked charts (If you don't know what is it, google
  /// it), you can fill up the [rodStackItems] to have a Stacked Chart.
  ...
  BarChartRodData({
    ...
    List<BarChartRodStackItem>? rodStackItems,
        ...
        width = width ?? 8,
        borderRadius =
            ChartUtils().normalizeBorderRadius(borderRadius, width ?? 8),
        borderSide = ChartUtils().normalizeBorderSide(borderSide, width ?? 8),
...
///
/// Each [BarChartRodData] can have a list of [BarChartRodStackItem] (with
/// different colors and position) to represent a Stacked Chart rod,
class BarChartRodStackItem with EquatableMixin {
  ...
  BarChartRodStackItem(
    ...
    this.toY,
    this.color, [
    this.borderSide = ChartUtils.defaultBorderSide,
  ]);
...
///
/// If you want to have a bar drawn in rear of the main rod, use
/// [BarChartRodData.backDrawRodData], it uses to have a bar with a passive
/// color in rear of the rod, for example you can use it as the maximum value
/// place holder in rear of your rod.
class BackgroundBarChartRodData with EquatableMixin {
  ...
  /// Lerp a [BackgroundBarChartRodData] based on [t] value, check [Tween.lerp].
...
/// into a concrete [BarTouchResponse].
class BarTouchData extends FlTouchData<BarTouchResponse> with EquatableMixin {
  ...
  ///
  /// if [handleBuiltInTouches] is true, [BarChart] shows a tooltip popup on
  /// top of the bars if touch occurs (or you can show it manually using,
  /// [BarChartGroupData.showingTooltipIndicators]), You can customize this
  /// tooltip using [touchTooltipData].
  /// If you need to have a distance threshold for handling touches, use
  /// [touchExtraThreshold]. If [allowTouchBarBackDraw] sets to true, touches
  /// will work on [BarChartRodData.backDrawRodData] too (by default it only
  /// works on the main rods).
  BarTouchData({
    bool? enabled,
...
class BarTouchTooltipData with EquatableMixin {
  ...
  /// [BarChart] shows a tooltip popup on top of rods automatically when touch
  /// happens, otherwise you can show it manually using
  /// [BarChartGroupData.showingTooltipIndicators].
  ...
  /// Content of the tooltip will provide using [getTooltipItem] callback, you
...
/// Provides a [BarTooltipItem] for showing content inside the
/// [BarTouchTooltipData].
///
/// You can override [BarTouchTooltipData.getTooltipItem], it gives you
/// [group], [groupIndex], [rod], and [rodIndex] that touch happened on,
/// then you should and pass your custom [BarTooltipItem] to show inside the
/// tooltip popup.
...
class BarTouchResponse extends BaseTouchResponse {
  /// If touch happens, [BarChart] processes it internally and passes out a
  /// BarTouchedSpot that contains a [spot], it gives you information about the
  /// touched spot.
...
class BarTouchedSpot extends TouchedSpot with EquatableMixin {
  ...
  /// [touchedBarGroup], and [touchedBarGroupIndex] tell you in which group
  /// touch happened,
  /// [touchedRodData], and [touchedRodDataIndex] tell you in which rod touch
  /// happened,
  /// [touchedStackItem], and [touchedStackItemIndex] tell you in which rod
  /// stack touch happened ([touchedStackItemIndex] means nothing found).
  /// You can also have the touched x and y in the chart as a [FlSpot] using
  /// [spot] value, and you can have the local touch coordinates on the screen
  /// as a [Offset] using [offset] value.
  BarTouchedSpot(
...
/// It lerps a [BarChartData] to another [BarChartData]
/// (handles animation for updating values)
