...
import '../base/axis_chart/axis_chart_extensions.dart';
...
import 'line_chart_renderer.dart' show RenderLineChart;
...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  /// Paints [RenderLineChart.data] into canvas, it is the animating
  /// [LineChartData], [RenderLineChart.targetData] is the animation's target
  /// and remains the same during animation, then we should use it  when we
  /// need to show tooltips or something like that, because
  /// [RenderLineChart.data] is changing constantly.
  ///
  /// [RenderLineChart.textScaler] used for scaling texts inside the chart,
  /// parent can use [MediaQueryData.textScaler] to respect the system's font
  /// size.
  ...
  void paint(
    ...
    for (var i = 0; i < data.lineBarsData.length; i++) {
      ...
      drawBarLine(canvasWrapper, barData, holder, primary);
      ...
      final indicatorsData = data.lineTouchData
          .getTouchedSpotIndicator(context, barData, barData.showingIndicators);
    ...
    if (data.showingTooltipIndicators != null) {
  ...
  void drawBarLine(
    ...
    for (final bar in barList) {
      ...
      drawBar(canvasWrapper, barPath, barData, holder, defaultColor);
    }
  }
  ...
  void drawBetweenBarsArea(
    ...
    for (var i = 0; i < fromBarSplitLines.length; i++) {
      ...
      final aroundRect = Rect.fromLTRB(
        _getPixelX(left, viewSize, holder),
        getPixelY(top, viewSize, holder),
        _getPixelX(right, viewSize, holder),
  ...
  void drawDots(
    ...
    for (var i = 0; i < barData.spots.length; i++) {
      ...
      if (spot.isNotNull() && barData.dotData.checkToShowDot(spot, barData)) {
        final x = _getPixelX(spot.x, viewSize, holder);
  ...
  void drawTouchedSpotsIndicator(
    ...
    for (final info in lineIndexDrawingInfo) {
      ...
      final touchedSpot = Offset(
        _getPixelX(spot.x, viewSize, holder),
      ...
      if (showingDots) {
        ...
        dotPainter = indicatorData.touchedSpotDotData
            .getDotPainter(context, spot, xPercentInLine, barData, index);
      ...
      /// If line end is inside the dot, adjust it so that it doesn't overlap
      /// with the dot.
  ...
  /// Generates a path, based on LineChartBarData.isStepChart for step style,
  /// and normal style.
  @visibleForTesting
  ...
  /// if cutOffY is provided by the [BarAreaData], it cut the area to the
  /// provided cutOffY value, if [fillCompletely] is true, the cutOffY will
  /// be ignored, and a completely filled path will return,
  @visibleForTesting
  Path generateBelowBarPath(
    ...
    var x = _getPixelX(barSpots[barSpots.length - 1].x, viewSize, holder);
    double y;
    ...
    /// Line To Bottom Left
    x = _getPixelX(barSpots[0].x, viewSize, holder);
    if (!fillCompletely && barData.belowBarData.applyCutOffY) {
      y = getPixelY(barData.belowBarData.cutOffY, viewSize, holder);
    } else {
      y = viewSize.height;
  ...
  /// firstly we draw [belowBarPath], then if cutOffY value is provided in
  /// [BarAreaData], [belowBarPath] maybe draw over the main bar line,
  /// then to fix the problem we use [filledAboveBarPath] to clear the above
  /// section from this draw.
  @visibleForTesting
  void drawBelowBar(
    CanvasWrapper canvasWrapper,
    Path belowBarPath,
    ...
    final belowBarLargestRect = Rect.fromLTRB(
      _getPixelX(barData.mostLeftSpot.x, viewSize, holder),
      getPixelY(barData.mostTopSpot.y, viewSize, holder),
      _getPixelX(barData.mostRightSpot.x, viewSize, holder),
      viewSize.height,
    );

    final belowBar = barData.belowBarData;
    _barAreaPaint.setColorOrGradient(
      belowBar.color ?? defaultColor,
      belowBar.gradient,
      belowBarLargestRect,
    );
  ...
  /// firstly we draw [aboveBarPath], then if cutOffY value is provided in
  /// [BarAreaData], [aboveBarPath] maybe draw over the main bar line,
  /// then to fix the problem we use [filledBelowBarPath] to clear the above
  /// section from this draw.
  @visibleForTesting
  void drawAboveBar(
    CanvasWrapper canvasWrapper,
  ...
  void drawBarShadow(
    ...
    _barPaint
      ...
      ..maskFilter = MaskFilter.blur(
        BlurStyle.normal,
        ChartUtils().convertRadiusToSigma(barData.shadow.blurRadius),
      );
  ...
  void drawTouchTooltip(
    ...
    for (var i = 0; i < showingTooltipSpots.showingSpots.length; i++) {
      ...
      final span = TextSpan(
        style:
            ChartUtils().getThemeAwareTextStyle(context, tooltipItem.textStyle),
    ...
    final LineTooltipTitle? title =
        tooltipData.getTitle?.call(showingTooltipSpots.showingSpots);
    if (title != null) {
      final TextSpan span = TextSpan(
        style: ChartUtils().getThemeAwareTextStyle(context, title.textStyle),
        text: title.text,
      );
    ...
    for (int i = 0; i < drawingTextPainters.length; i++) {
      final tp = drawingTextPainters[i];
      final FlTooltipIndicator? indicator = tooltipItems[i]?.indicator;
    ...
    final mostTopOffset = Offset(
      _getPixelX(showOnSpot.x, viewSize, holder),
    ...
    double tooltipLeftPosition = getTooltipLeft(
      mostTopOffset.dx,
      tooltipWidth,
    ...
    _bgTouchTooltipPaint.color =
        tooltipData.tooltipBgColor ?? Theme.of(context).colorScheme.surface;

    final rotateAngle = tooltipData.rotateAngle;
    final rectRotationOffset = Offset(
      0,
      ChartUtils().calculateRotationOffset(rect.size, rotateAngle).dy,
    );
    final rectDrawOffset = Offset(roundedRect.left, roundedRect.top);

    final textRotationOffset =
        ChartUtils().calculateRotationOffset(rect.size, rotateAngle);
  ...
  void drawTooltipIndicator(
    ...
    final double width = indicator.width;
    final double height = indicator.height;
    final double radius = max(width, height) / 2;
    final Rect rect = Rect.fromLTWH(offset.dx, offset.dy, width, height);
  ...
  double getBarLineXLength(
    ...
    final firstSpotX = _getPixelX(firstSpot.x, chartUsableSize, holder);

    final lastSpot = barData.spots[barData.spots.length - 1];
    final lastSpotX = _getPixelX(lastSpot.x, chartUsableSize, holder);
  ...
  double _getPixelX(
    ...
    return super.getPixelX(spotX, viewSize, holder) + holder.data.horizontalGap;
  }
}
