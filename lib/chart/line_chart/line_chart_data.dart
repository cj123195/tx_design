...
import '../bar_chart/bar_chart.dart' show BarChart;
...
import '../base/base_chart/fl_touch_event.dart'
    show FlTouchEvent, FlPointerHoverEvent, FlTapUpEvent;
import '../lerp.dart';
import 'line_chart.dart' show LineChart;
...
class LineChartData extends AxisChartData with EquatableMixin {
  ...
  /// lines are defined in [lineBarsData], sometimes you need to fill space
  /// between two bars with a color or gradient, you can use [betweenBarsData]
  /// to achieve that.
  ///
  /// It draws some titles on left, top, right, bottom sides per each axis
  /// number, you can modify [titlesData] to have your custom titles,
  /// also you can define the axis title (one text per axis) for each side
  /// using axisTitleData, you can restrict the y axis using [minY] and [maxY]
  /// value, and restrict x axis using [minX] and [maxX].
  ///
  /// It draws a color as a background behind everything you can set it using
  /// [backgroundColor], then a grid over it, you can customize it using
  /// [gridData], and it draws 4 borders around your chart, you can customize
  /// it using [borderData].
  ///
  /// You can annotate some regions with a highlight color using
  /// [rangeAnnotations].
  ///
  ...
  /// just put line indicator number and spots indices you want to show it on
  /// top of them.
  ///
  /// [LineChart] draws some horizontal or vertical lines on above or below of
  /// everything, they are useful in some scenarios, for example you can show
  /// average line, you can fill [extraLinesData] property to have your extra
  /// lines.
  ///
  /// [clipData] forces the [LineChart] to draw lines inside the chart bounding
  /// box.
  LineChartData({
    this.lineBarsData = const [],
    this.betweenBarsData,
    ...
    super.horizontalGap,
        ...
        super(
          ...
          gridData: gridData ?? const FlGridData(show: false),
...
class LineChartBarData with EquatableMixin {
  ...
  /// [isCurved] makes it curve for drawing, and [curveSmoothness] determines
  /// the curve smoothness.
  ///
  ...
  ///
  /// if [isCurved] is true, in some situations if the spots changes are in
  /// high values, an overshooting will happen, we don't have any idea to solve
  /// this at the moment, but you can set [preventCurveOverShooting] true, and
  ...
  ///
  /// [belowBarData], and  [aboveBarData] used to fill the space below or above
  /// the drawn line,
  ...
  /// true, also you can tweak the [LineChartBarData.lineChartStepData].
  LineChartBarData({
    this.spots = const [],
    this.show = true,
    this.color,
    ...
    this.dotData = const FlDotData(show: false),
  ...
  /// Determines the dash length and space respectively, fill it if you want to
  /// have dashed line.
  ...
  /// If sets true, it draws the chart in Step Line Chart style, using
  /// [LineChartBarData.lineChartStepData].
  final bool isStepLineChart;

  /// Holds data for representing a Step Line Chart, and works only if
  /// [isStepLineChart] is true.
...
/// Holds data for representing a Step Line Chart, and works only if
/// [LineChartBarData.isStepLineChart] is true.
class LineChartStepData with EquatableMixin {
  ...
  /// Go to the half with the current spot y, and with the next spot y for the
  /// rest.
...
/// Holds data for filling an area (above or below) of the line with a color
/// or gradient.
class BarAreaData with EquatableMixin {
  ...
  /// otherwise it gradients between provided colors for drawing the line.
  /// Gradient happens using provided gradientColorStops, gradientFrom,
  /// gradientTo.
  /// if you want it draw normally, don't touch them,
  ...
  /// holds data for drawing a line from each spot the the bottom, or top of
  /// the chart
...
class BarAreaSpotsLine with EquatableMixin {
  /// If [show] is true, [LineChart] draws some lines on above or below the
  /// spots, you can customize the appearance of the lines using [flLineStyle]
  /// and you can decide to show or hide the lines on each spot using
  /// [checkToShowSpotLine].
...
/// in a darker mode, otherwise it returns the color along the gradient colors
/// based on the [xPercentage] in a darker mode.
Color? _defaultGetDotStrokeColor(
  ...
  if (bar.gradient != null && bar.gradient is LinearGradient) {
    ...
    color = bar.gradient?.colors.first ?? bar.color;
  }
  return color;
}
...
/// into a concrete [LineTouchResponse].
class LineTouchData extends FlTouchData<LineTouchResponse> with EquatableMixin {
  ...
  ///
  /// if [handleBuiltInTouches] is true, [LineChart] shows a tooltip popup on
  /// top of the spots if touch occurs (or you can show it manually using,
  /// [LineChartData.showingTooltipIndicators]) and also it shows an indicator
  /// (contains a thicker line and larger dot on the targeted spot), You can
  /// define how this indicator looks like through [getTouchedSpotIndicator]
  /// callback, You can customize this tooltip using [touchTooltipData],
  /// indicator lines starts from position controlled by [getTouchLineStart]
  /// and ends at position controlled by [getTouchLineEnd].
  /// If you need to have a distance threshold for handling touches, use
  /// [touchSpotThreshold].
  const LineTouchData({
    bool enabled = true,
  ...
  /// The starting point on y axis of the touch line. By default, line starts
  /// on the bottom of the chart.
  final GetTouchLineY getTouchLineStart;

  /// The end point on y axis of the touch line. By default, line ends at the
  /// touched point.
  /// If line end is overlap with the dot, it will be automatically adjusted to
  /// the edge of the dot.
...
/// Used for showing touch indicators (a thicker line and larger dot on the
/// targeted spot).
///
/// It gives you the [spotIndexes] that touch happened, or manually targeted,
/// in the given [barData], you should return a list of
/// [TouchedSpotIndicatorData], length of this list should be equal to the
/// spotIndexes.length, each [TouchedSpotIndicatorData] determines the look
/// of showing indicator.
...
/// Used to calculate the distance between coordinates of a touch event and a
/// spot
...
List<TouchedSpotIndicatorData> defaultTouchedIndicators(
  ...
  return indicators.map((int index) {
    ...
    final dotData = FlDotData(
      getDotPainter: (context, spot, percent, bar, index) =>
...
class LineTouchTooltipData with EquatableMixin {
  ...
  /// [LineChart] shows a tooltip popup on top of spots automatically when
  /// touch happens, otherwise you can show it manually using
  /// [LineChartData.showingTooltipIndicators].
  /// Tooltip shows on top of spots, with [tooltipBgColor] as a background
  /// color, and you can set corner radius using [tooltipRoundedRadius].
  /// If you want to have a padding inside the tooltip, fill [tooltipPadding],
  /// or If you want to have a bottom margin, set [tooltipMargin].
  /// Content of the tooltip will provide using [getTooltipItems] callback, you
  /// can override it and pass your custom data to show in the tooltip.
  /// You can restrict the tooltip's width using [maxContentWidth].
  /// Sometimes, [LineChart] shows the tooltip outside of the chart,
  /// you can set [fitInsideHorizontally] true to force it to shift inside the
...
class LineBarSpot extends FlSpot with EquatableMixin {
  ...
  /// [barIndex] is the index of our [bar], in the [LineChartData.lineBarsData]
  /// list, [spot] is the targeted spot.
  /// [spotIndex] is the index this [FlSpot], in the [LineChartBarData.spots]
  /// list.
  LineBarSpot(
    this.bar,
    this.barIndex,
...
class TouchedSpotIndicatorData with EquatableMixin {
  /// if [LineTouchData.handleBuiltInTouches] is true,
  /// [LineChart] shows a thicker line and larger spot as indicator
  /// automatically when touch happens, otherwise you can show it manually
  /// using [LineChartBarData.showingIndicators].
...
class LineTouchResponse extends BaseTouchResponse {
  /// If touch happens, [LineChart] processes it internally and passes out a
  /// list of [lineBarSpots] it gives you information about the touched spot.
  ...
  /// (if a single line provided on the chart, [lineBarSpots]'s length will be
  /// 1 always)
...
/// It lerps a [LineChartData] to another [LineChartData] (handles animation
/// for updating values)
